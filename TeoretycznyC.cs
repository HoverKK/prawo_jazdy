...
{
    public partial class TeoretycznyC : Form
    {
        ...
        private List<QuestionC> questions = new List<QuestionC>();
        private List<QuestionC> selectedQuestions = new List<QuestionC>();
        ...
        private void LoadQuestionsFromDatabase()
        {
            using (SqlConnection connection = new SqlConnection(@"Data Source=DESKTOP-ED41F2S;Initial Catalog=Prawo_jazdy;Integrated Security=True"))
            {
                ...
                Dictionary<int, QuestionC> questionDictionary = new Dictionary<int, QuestionC>();
                ...
                while (reader.Read())
                {
                    ...
                    {
                        questionDictionary[questionId] = new QuestionC
                        {
                            QuestionID = questionId,
                            QuestionText = questionText,
                            Points = points,
                            Answers = new List<AnswerC>()
                        };
                    }

                    questionDictionary[questionId].Answers.Add(new AnswerC
                    {
                        AnswerID = answerId,
        ...
        private void CheckAnswer()
        {
            if (selectedQuestions.Count > 0 && currentQuestionIndex < selectedQuestions.Count)
            {
                ...
                currentQuestion.UserSelectedAnswer = selectedAnswerText;
        ...
        {
            var resultForm = new ResultFormC(correctAnswersCount, selectedQuestions.Count, totalPoints, maxPoints, selectedQuestions);
            resultForm.Show();
            this.Close();
        }

        private void TeoretycznyC_Load(object sender, EventArgs e)
        {
            DisplayQuestion();
        }
    }
...
public class QuestionC
{
    ...
    public List<AnswerC> Answers { get; set; }
...
public class ResultFormC : Form
{
    public ResultFormC(int correctAnswersCount, int totalQuestions, int totalPoints, int maxPoints, List<QuestionC> questions)
    {
        this.Text = "Wynik";
